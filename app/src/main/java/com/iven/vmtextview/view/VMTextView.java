package com.iven.vmtextview.view;
...
import com.iven.vmtextview.R;
import com.iven.vmtextview.entity.VMEntity;
import com.iven.vmtextview.util.SizeUtil;
 ...
 * @author Iven
 * @version 1.0
 * @Date 2016/8/14 18:36
 * Email: yanfengqiu06@163.com
 */
public class VMTextView extends View{
    ...
    private List<VMEntity> mTexts; //显示文字的数据源
    ...
    public VMTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //取得资源attrs.xml中的资源值
    ...
    private void obtainStyledAttrs(AttributeSet attrs) {
        //获得自定义的attrs.xml中的资源属性
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.VMTextView);
        mSpeed = array.getInt(R.styleable.VMTextView_vm_text_view_speed, 1);
        mInterval = array.getInt(R.styleable.VMTextView_vm_text_view_interval, 2000);
        mFrontColor = array.getColor(R.styleable.VMTextView_vm_text_front_color, Color.RED);
        mContentColor = array.getColor(R.styleable.VMTextView_vm_text_content_color, Color.BLACK);
        mFrontTextSize = (int) array.getDimension(R.styleable.VMTextView_vm_text_front_size, SizeUtil.Sp2Px(getContext(), 15));
        mContentTextSize = (int) array.getDimension(R.styleable.VMTextView_vm_text_content_size, SizeUtil.Sp2Px(getContext(), 15));
        array.recycle();//切记一定调用这个方法！！！
    }
        ...
        switch (action){
            case MotionEvent.ACTION_DOWN:
                if (OnItemClickListener != null) {
                    OnItemClickListener.onClick(mTexts.get(mIndex).getURL());
                }
                break;
        }
    ...
    //设置数据源
    public void setmTexts(List<VMEntity> mTexts) {
        this.mTexts = mTexts;
    }
    //设置广告文字的停顿时间
    ...
    protected void onDraw(Canvas canvas) {
        if (mTexts != null) {
            VMEntity model = mTexts.get(mIndex);
